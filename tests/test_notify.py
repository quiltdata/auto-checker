...
# --- a no-op re-push says so -------------------------------------------------

def test_identical_republish_is_noted(policy, tmp_path):
    """Two pointers may name one manifest. The revision then has nothing to
    compare against, and the report says so rather than leaving a reader to
    infer a comparison from prev_tophash == tophash."""
    from check_commit.engine import Context, run
    from check_commit.model import RevisionView
    ...
    prev = RevisionView(**{**view.__dict__, "pointer": "1789193859"})
    ctx = Context(FakeHistory(), [], online=False, policy=policy)
    report = run(prev, view, ctx)
    assert any("re-publishes the manifest" in n for n in report.notes)
    body = notify.render(report, policy)
    assert "## Notes" in body
    assert "re-publishes the manifest" in body
